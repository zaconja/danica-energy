...
class MILPDayAheadOptimizer:
    ...
    def optimize(self, initial_soc=0.0):
        ...
        # 3. Dinamika baterije – popravljeno: umjesto dijeljenja koristi množenje s 1/eff
        prob += soc[0] == initial_soc, "soc_initial"
        for t in range(T-1):
            prob += (soc[t+1] == soc[t] + self.eff * ch[t] - dis[t] * (1.0 / self.eff)), f"soc_dynamic_{t}"

        # 4. Ograničenja baterije
        for t in range(T):
